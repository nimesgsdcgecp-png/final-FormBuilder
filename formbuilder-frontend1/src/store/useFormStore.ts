...
interface FormState {
  ...
  moveField: (fieldId: string, targetParentId: string | null, targetIndex: number) => void;
...
export const useFormStore = create<FormState>((set) => ({
  ...
  /**
   * Moves an existing field from its current location to a new parent/index.
   * Handles root→section, section→root, and section→section cross-container moves.
   */
  moveField: (fieldId, targetParentId, targetIndex) =>
    set((state) => {
      const field = findFieldInTree(state.schema.fields, fieldId);
      if (!field) return state;
      // Remove from current location
      const withoutField = removeFieldFromTree(state.schema.fields, fieldId);
      // Insert at new location
      const updatedFields = insertFieldIntoTree(withoutField, field, targetIndex, targetParentId);
      return { schema: { ...state.schema, fields: updatedFields } };
    }),
}));
