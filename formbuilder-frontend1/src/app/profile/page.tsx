          ...
          <div className="grid gap-6">
            ...
            <section className="p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border bg-[var(--card-bg)] border-[var(--card-border)] shadow-xl overflow-hidden relative">
              ...
              <form onSubmit={handleUpdate} className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-[var(--text-faint)]">First Name</label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-[var(--text-faint)]">Last Name</label>
                  <input
                    type="text"
                    className="w-full px-5 py-4 rounded-2xl border bg-[var(--bg-muted)] transition-all focus:ring-2 focus:ring-[var(--accent)] outline-none font-bold"
                    style={{ borderColor: 'var(--border)' }}
                ...
                <div className="space-y-2 sm:col-span-2">
                  <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-[var(--text-faint)]">Email Address</label>
                  <input
                    type="email"
                    className="w-full px-5 py-4 rounded-2xl border bg-[var(--bg-muted)] transition-all focus:ring-2 focus:ring-[var(--accent)] outline-none font-bold"
                    style={{ borderColor: 'var(--border)' }}
            ...
            <section className="p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border bg-[var(--card-bg)] border-[var(--card-border)] shadow-xl overflow-hidden relative">
              <div className="flex items-center gap-3 mb-6 sm:mb-8">
                ...
                <h2 className="text-lg sm:text-xl font-black tracking-tight">Security Credentials</h2>
              </div>

              <form onSubmit={handleUpdate} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-[var(--text-faint)]">New Password</label>
                  <div className="relative">
                    <input
                      type="password"
                      placeholder="••••••••"
                      className="w-full px-5 py-4 rounded-2xl border bg-[var(--bg-muted)] transition-all focus:ring-2 focus:ring-[var(--accent)] outline-none font-bold placeholder:text-[var(--text-faint)]"
                      style={{ borderColor: 'var(--border)' }}
                      value={formData.password}
                      onChange={e => setFormData({ ...formData, password: e.target.value })}
                    />
                  ...
                  <p className="text-[9px] text-[var(--text-muted)] ml-1 font-medium italic">Leave blank if you don't want to change your password.</p>
                </div>

                <div className="pt-4">
                  <button
                    type="submit"
                    disabled={isUpdating || !formData.password}
                    className="w-full py-4 rounded-2xl text-sm font-black text-[var(--text-primary)] border border-[var(--border)] bg-[var(--bg-muted)] hover:bg-[var(--card-border)] transition-all flex items-center justify-center gap-2 group disabled:opacity-50"
                  >
                    {isUpdating ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle2 size={20} />}
                    Update Password
                  </button>
