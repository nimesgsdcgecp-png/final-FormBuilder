...
public class DynamicTableService {
    ...
    public List<String> detectSchemaDrift(String tableName, List<com.sttl.formbuilder2.model.entity.FormField> fields) {
        ...
        for (com.sttl.formbuilder2.model.entity.FormField field : fields) {
            ...
            // Skip UI-only fields that do not have database columns
            if (mapToSqlType(field.getFieldType()) == null) {
                continue;
            }
    ...
    /**
     * Checks whether a given value already exists in a specific column of the form's
     * dynamic data table. Excludes soft-deleted records and draft rows so that
     * only live, submitted data is compared.
     *
     * @param tableName  The form's dynamic data table name (e.g. "form_data_registration")
     * @param columnName The field's snake_case column name to check
     * @param value      The submitted value to look for
     * @return true if another non-deleted, non-draft row already has this value
     */
    public boolean existsValueInColumn(String tableName, String columnName, String value) {
