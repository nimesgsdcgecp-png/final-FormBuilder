...
public class FormService {
    ...
    private String resolveUniqueFormCode(String baseCode) {
        String code = baseCode;
        int counter = 1;
        while (formRepository.existsByCode(code)) {
            if (counter > 10) {
                throw new FormBuilderException("DUPLICATE_FORM_CODE", "Form code already exists and could not be auto-resolved: " + baseCode);
            }
            code = baseCode + "_" + counter++;
            if (code.length() > 100) {
                code = baseCode.substring(0, 95) + "_" + counter;
            }
        }
        return code;
    }
    ...
    public FormDetailResponseDTO createForm(CreateFormRequestDTO request) {
        ...
        String finalCode = resolveUniqueFormCode(request.getCode());
        form.setCode(finalCode);
    ...
    public FormDetailResponseDTO getFormByCode(String code) {
        ...
        if (form == null) {
            throw new FormBuilderException("FORM_NOT_FOUND", "Form not found with code or ID: " + code);
        }
